...
public class ERNurseAgent extends Agent
{
	...
	private InitSimParam initSimParam;							// 初期設定ファイル操作クラス
...
//		lfRand = weibullRand(initSimParam.lfGetNurseJudgeMildTraumaWeibullAlpha(), initSimParam.lfGetNurseJudgeMildTraumaWeibullBeta());
//		if( lfRand <= lfPrevProb )
//		{
//			iEmergency = 4;
//		}
//		lfCurProb = initSimParam.lfGetNurseJudgeMildTrauma4();
...
//		lfCurProb += 0.5;
//		lfCurProb += initSimParam.lfGetNurseJudgeMildTrauma5();
//		if( lfPrevProb <= lfRand && lfRand <= lfCurProb )
		{
			iEmergency = 5;
		}
...
//		lfRand = weibullRand(1.8, 0.35);			// 現在設定値
//		lfRand = weibullRand(2.5, 0.4);	//O.Kデータ
//		lfRand = weibullRand(4.0, 0.4);
		lfRand = weibullRand(initSimParam.lfGetNurseJudgeModerateTraumaWeibullAlpha(), initSimParam.lfGetNurseJudgeModerateTraumaWeibullBeta());
		if( lfRand <= lfPrevProb )
		{
			iEmergency = 3;
		}
		lfCurProb = initSimParam.lfGetNurseJudgeModerateTrauma3();
		if( lfPrevProb <= lfRand && lfRand <= lfCurProb )
		{
			iEmergency = 3;
		}
		lfPrevProb = lfCurProb;
//		lfCurProb += 0.5;
		lfCurProb += initSimParam.lfGetNurseJudgeModerateTrauma4();
		if( lfPrevProb <= lfRand && lfRand <= lfCurProb )
		{
			iEmergency = 4;
		}
		lfPrevProb = lfCurProb;
//		lfCurProb += 0.25;
		lfCurProb += initSimParam.lfGetNurseJudgeModerateTrauma5();
		if( lfPrevProb <= lfRand && lfRand <= lfCurProb )
		{
			iEmergency = 5;
		}
...
//		lfRand = weibullRand(2.5, 0.4);	//O.Kデータ	//現在設定値
		lfRand = weibullRand(initSimParam.lfGetNurseJudgeSevereTraumaWeibullAlpha(), initSimParam.lfGetNurseJudgeSevereTraumaWeibullBeta());
		if( lfRand <= lfPrevProb )
		{
			iEmergency = 2;
		}
		lfCurProb = initSimParam.lfGetNurseJudgeSevereTrauma2();
		if( lfPrevProb <= lfRand && lfRand <= lfCurProb )
		{
			iEmergency = 2;
		}
		lfPrevProb = lfCurProb;
//		lfCurProb += 0.5;
		lfCurProb += initSimParam.lfGetNurseJudgeSevereTrauma3();
		if( lfPrevProb <= lfRand && lfRand <= lfCurProb )
		{
			iEmergency = 3;
		}
		lfPrevProb = lfCurProb;
//		lfCurProb += 0.25;
		lfCurProb += initSimParam.lfGetNurseJudgeSevereTrauma4();
		if( lfPrevProb <= lfRand && lfRand <= lfCurProb )
		{
			iEmergency = 4;
		}
