...
public class ERExaminationCTRoom extends Agent
{
	...
	double lfTotalTime;
	private Logger cCTRoomLog;											// ログクラス
	Rand rnd;															// 乱数クラス
	...
	private Object csCTRoomCriticalSection;								// クリティカルセクション用

	private double lfLastBedTime;
	private double lfLongestTotalTime;

	public ERExaminationCTRoom()
	{
		vInitialize();
	}
	 ...
	 * @since 2015/08/07
	 */
	public ERExaminationCTRoom( int iAttachedClinicalEngineerNumData )
	{
...
//		rnd = new Sfmt( (int)seed );
	...
	public synchronized void vTerminate() throws IOException
	{
		...
		synchronized( csCTRoomCriticalSection )
		{
			// 患者エージェントの終了処理を行います。
...
//			ArrayListClinicalEngineerAgents.get(i).vSetExaminationTime( alfExaminationTime[i] );
	...
	public void vImplementExaminationRoom( ArrayList<EREmergencyRoom> ArrayListEmergencyRooms, ArrayList<ERSevereInjuryObservationRoom> ArrayListSereveInjuryObservationRooms, ArrayList<ERConsultationRoom> ArrayListConsultationRooms, ArrayList<EROperationRoom> ArrayListOperationRooms, ERWaitingRoom erWaitingRoom ) throws ERClinicalEngineerAgentException
	{
		synchronized( csCTRoomCriticalSection )
		{
			...
			if( erCurrentPatientAgent != null )
			{
				cCTRoomLog.info(erCurrentPatientAgent.getId() +"," + "CT室対応中" );
	...
	private void vImplementExamination( ArrayList<EREmergencyRoom> ArrayListEmergencyRooms, ArrayList<ERSevereInjuryObservationRoom> ArrayListSereveInjuryObservationRooms, ArrayList<ERConsultationRoom> ArrayListConsultationRooms, ArrayList<EROperationRoom> ArrayListOperationRooms, ERWaitingRoom erWaitingRoom, ERClinicalEngineerAgent erClinicalEngineerAgent, ERPatientAgent erPatientAgent ) throws ERClinicalEngineerAgentException
	{
		...
		if( erCurrentPatientAgent.isMoveWaitingTime() == false )
		{
			...
			cCTRoomLog.info( erPatientAgent.getId() + "," + "CT室移動時間：" + erCurrentPatientAgent.lfGetMoveWaitingTime() );
			return;
		}
...
//		if( erClinicalEngineerAgent.lfGetExaminationTime() > erClinicalEngineerAgent.lfGetCurrentPassOverTime() )
		{
			// 何もせずに終了します。
			cCTRoomLog.info( erPatientAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "CT検査時間：" + erClinicalEngineerAgent.lfGetExaminationTime() );
			cCTRoomLog.info( erPatientAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "CT対応時間：" + erClinicalEngineerAgent.lfGetCurrentPassOverTime() );
			return ;
		}
		// CT室（）の検査を実行します。
		erClinicalEngineerAgent.vImplementExamination( erPatientAgent, 3 );
		cCTRoomLog.info( erPatientAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "CT検査終了" );
	...
	private void vJudgeMoveConsultationRoom( ArrayList<ERConsultationRoom> ArrayListConsultationRooms, ERWaitingRoom erWaitingRoom, ERClinicalEngineerAgent erClinicalEngineerAgent, ERPatientAgent erPAgent  ) throws ERClinicalEngineerAgentException
	{
		...
		for( i = 0;i < ArrayListConsultationRooms.size(); i++ )
		{
			...
			{
				cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "診察室へ移動準備開始" + "," + "CT室" );
				// 診察室待機フラグをOFFにします。
			...
			// 看護師、および医師エージェントにメッセージを送信します。
				...
				if( iInverseSimFlag == 1 )
				{
					...
					cCTRoomLog.info( erPAgent.getId() + "," + this.erGetTriageNode() + "," +  erWaitingRoom.erGetTriageNode() );
					cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "診察室へ移動開始" + "," + "CT室" );
				}
				cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "診察室へ移動準備終了" + "," + "CT室" );
...
//				ArrayListConsultationRooms.get(i).cGetDoctorAgent().vSetConsultation(1);
		...
		{
			// 診察室待機フラグをONにします。
			cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "診察室満室" + "," + "CT室" );
	...
	private void vJudgeMoveOperationRoom( ArrayList<EROperationRoom> ArrayListOperationRooms, ERWaitingRoom erWaitingRoom, ERClinicalEngineerAgent erClinicalEngineerAgent, ERPatientAgent erPAgent )
	{
		...
		for( i = 0;i < ArrayListOperationRooms.size(); i++ )
		{
			...
			{
				cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "手術室へ移動準備開始" + "," + "CT室" );
				// 手術室待ちフラグをOFFにします。
				...
				cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "手術室へ移動準備終了" + "," + "CT室" );
				if( iInverseSimFlag == 1 )
				{
					// 移動先の経路を患者エージェントに設定します。
					erPAgent.vSetMoveRoute( erTriageNodeManager.getRoute( this.erGetTriageNode(), ArrayListOperationRooms.get(i).erGetTriageNode() ) );
					cCTRoomLog.info( erPAgent.getId() + "," + this.erGetTriageNode() + "," +  erWaitingRoom.erGetTriageNode() );
					cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "手術室へ移動開始" + "," + "CT室" );
				}
				erPAgent = null;
		...
		if( iJudgeCount == ArrayListOperationRooms.size() )
		{
			...
			cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "手術室満室" + "," + "CT室" );
...
//		ERClinicalEngineerAgent erEmergencyclinicalEngineerAgent;
		for( i = 0;i < ArrayListEmergencyRooms.size(); i++ )
		{
			...
			{
				cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "初療室へ移動準備開始" + "," + "CT室" );
				// 初療室待ちフラグをOFFにします。
				...
				{
					erEmergencyNurseAgent = ArrayListEmergencyRooms.get(i).cGetNurseAgent(i);
...
//				}
				...
				if( iInverseSimFlag == 1 )
				{
					...
					cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "初療室へ移動開始" + "," + "CT室" );
					cCTRoomLog.info( erPAgent.getId() + "," + this.erGetTriageNode() + "," +  erWaitingRoom.erGetTriageNode() );
				}
				cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "初療室へ移動準備終了" + "," + "CT室" );
		...
		if( iJudgeCount == ArrayListEmergencyRooms.size() )
		{
			...
			cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "初療室満室" + "," + "CT室" );
	...
	private void vJudgeMoveSereveInjuryObservationRoom( ArrayList<ERSevereInjuryObservationRoom> ArrayListSereveInjuryObservationRooms, ERWaitingRoom erWaitingRoom, ERClinicalEngineerAgent erClinicalEngineerAgent, ERPatientAgent erPAgent  )
	{
		...
		for( i = 0;i < ArrayListSereveInjuryObservationRooms.size(); i++ )
		{
			...
			{
				cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "重症観察室へ移動準備開始" + "," + "CT室" );
				// 重傷観察室待機フラグをOFFにします。
				...
				for( j = 0;j < ArrayListSereveInjuryObservationRooms.get(i).iGetNurseAgentsNum(); j++ )
				{
					...
					erSevereInjuryObservationNurseAgent = ArrayListSereveInjuryObservationRooms.get(i).erGetNurseAgent(i);
				...
				if( iInverseSimFlag == 1 )
				{
					...
					cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "重症観察室へ移動開始" + "," + "CT室" );
					cCTRoomLog.info( erPAgent.getId() + "," + this.erGetTriageNode() + "," +  erWaitingRoom.erGetTriageNode() );
				}
				cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "重症観察室へ移動準備終了" + "," + "CT室" );
		...
		if( iJudgeCount == ArrayListSereveInjuryObservationRooms.size() )
		{
			...
			cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "重症観察室満室" + "," + "CT室" );
	...
	private void vJudgeMoveWaitingRoom( ERWaitingRoom erWaitingRoom, ERClinicalEngineerAgent erClinicalEngineerAgent, ERPatientAgent erPAgent )
	{
		...
		{
			cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "待合室へ移動準備開始" + "," + "CT室" );
			// 患者エージェントを待合室に配置します。
			...
			if( iInverseSimFlag == 1 )
			{
				...
				cCTRoomLog.info( erPAgent.getId() + "," + this.erGetTriageNode() + "," +  erWaitingRoom.erGetTriageNode() );
				cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "待合室へ移動開始" + "," + "CT室"  );
			}
			cCTRoomLog.info( erPAgent.getId() + "," + erClinicalEngineerAgent.getId() + "," + "待合室へ移動準備終了" + "," + "CT室"  );
	...
	{
		if( erNodeManager == null )
		{
			System.out.println("erTriageNodeManagerがnull：vSetERTriageNodeManager時点");
			cCTRoomLog.info( "erTriageNodeManagerがnull：vSetERTriageNodeManager時点" );
		}
	 ...
	 *    現在選択されているCT室のノードを取得します。
	 * </PRE>
	 * @param erNode	設定するノードインスタンス(CT室)
	 */
