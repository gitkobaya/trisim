...
public class InitInverseSimParam extends InitSettingFileRead
{
	...
	// 制約条件設定
		...
		// 初期設定値をランダムに割り当てるかどうかを設定します。
		iInitializeGenerateMode = (int)GetInitDataInt( strInitializationSectionName, "InitializeGenerateMode", -1, strIniFullPath );
		if( iInitializeGenerateMode < 0 || iInitializeGenerateMode > 2 )
		{
			/* 範囲外を指定した場合はエラーを出力します。*/
			throw(new IllegalArgumentException("random generation flag is out of range"));
		}
	}
