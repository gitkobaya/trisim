...
//					// 初期化を実行します。
					vInitialize( cmd, cTRISimLogger, strNodeLinkFileName, csCriticalSection );
...
//					vThreadInvoke( engine, erDepartment );
			...
			else if( cmd.iGetExecMode() == 1 )
			{
				...
				// 初期化を実行します。
				vInitialize( erDepartment, cTRISimLogger, strNodeLinkFileName, csCriticalSection, initGuiSimParam );
...
//		}
	 ...
	 */
	private static void vInitialize( CCmdCheck cmd, ERDepartment erDepartment, Logger log, String strNodeLinkFileName, Object cs )
	{
		// 地形の作成(縦横のメッシュの数を指定。)
		...
		erDepartment.vInitialize( engine, env, cmd.strGetEmergencyDepartmentPath(), cmd.strGetConsultationRoomPath(),
				...
				cmd.iGetPatientArrivalMode(), random  );
...
//		erDepartment.vReadNodeManager( strNodeLinkFileName );
	 ...
	 */
	private static void vInitialize( CCmdCheck cmd, Logger log, String strNodeLinkFileName, Object cs ) throws IOException
	{
		// 地形の作成(縦横のメッシュの数を指定。)
		geo = new SimpleMeshGeometry(300, 300);
		...
		invSimEngine.vInitialize( cmd.iGetInverseSimulationIntervalNumber(), cmd.iGetGensNumber(), cmd.iGetGensVectorDimension(), cmd.iGetInverseSimulationMethod(),
			...
			cmd.strGetHighCareUnitPath(), cmd.strGetGeneralWardPath(), cmd.strGetWaitingRoomPath(), cmd.strGetXRayRoomPath(),
			cmd.strGetCTRoomPath(), cmd.strGetMRIRoomPath(), cmd.strGetAngiographyRoomPath(), cmd.strGetFastRoomPath(),
			log, cs, random, cmd.lfGetPatientPepole(), cmd.iGetPatientRandomMode(), cmd.iGetExecMode(), cmd.iGetFileWriteMode(),cmd.iGetPatientArrivalMode() );
...
//			invSimEngine.vInitInvSimEnginePso(iPsoMethodData, iSearchNumData, iLimitCountData);
	 ...
	 */
	private static void vInitialize( ERDepartment erDepartment, Logger log, String strNodeLinkFileName, Object cs, InitGUISimParam initGuiSimParam ) throws IOException
	{
